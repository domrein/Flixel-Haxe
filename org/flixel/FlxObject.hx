	...
	class FlxObject extends FlxRect {
		...
		function updateMotion():Void
		{
			...
			if(thrust != 0)
			{
				...
				var max:Int = Math.floor(((maxComponents.x>0)?maxComponents.x:-maxComponents.x));
			...
			vc = (FlxU.computeVelocity(velocity.x,Math.floor(acceleration.x+thrustComponents.x),Math.floor(drag.x),Math.floor(maxVelocity.x)) - velocity.x)/2;
			velocity.x += vc;
		...
		{
			getScreenXY(_point);
			var tx:Int = Math.floor(_point.x);
			var ty:Int = Math.floor(_point.y);
			var tw:Int = Math.floor(width);
			var th:Int = Math.floor(height);
			...
			var ox:Int = Math.floor(_point.x);
			var oy:Int = Math.floor(_point.y);
			var ow:Int = Math.floor(Object.width);
			var oh:Int = Math.floor(Object.height);
		 ...
		 */
		public function flicker(?Duration:Int=1):Void { _flickerTimer = Duration; if(_flickerTimer < 0) { _flicker = false; visible = true; } }
